...
const Router = {
  ...
  register(id, { title, load, destroy }) {
    this.pages[id] = { id, title, load, destroy };
  },
  ...
  async go(id, params = {}, opts = {}) {
    ...
    // save to stack
    if (!opts.replace) {
      this.stack.push({ id: this.current, params: this._currentParams });
    }
  ...
  async back() {
    if (this.stack.length === 0) {
      await this.go('home', {}, { replace: true });
      return;
    }
    const prev = this.stack.pop();
    if (prev?.id) {
      history.replaceState({ page: prev.id }, '', `#${prev.id}`);
      await this._render(this.pages[prev.id], prev.params || {});
    } else {
      await this.go('home', {}, { replace: true });
    }
  },
  ...
  init() {
    ...
    window.addEventListener('popstate', async e => {
      if (this.stack.length > 0) {
        const prev = this.stack.pop();
        if (prev?.id) await this._render(this.pages[prev.id], prev.params || {});
        else await this._render(this.pages['home'], {});
      } else {
        await this._render(this.pages['home'], {});
