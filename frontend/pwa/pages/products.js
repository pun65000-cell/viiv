/* VIIV PWA — products.js v2 — 4 tabs: สินค้า | หมวดหมู่ | รับสินค้า | จัดการ
 * ใช้ API เดียวกับ PC (/api/pos/products/*, /api/pos/categories/*, /api/pos/receive/*)
 * PC pages (iframe) ไม่สามารถ import shared module ได้ตอนนี้ — logic อยู่ใน IIFE นี้
 */
(function() {
  ...
  function _renderAll() {
    ...
    el.innerHTML = `
      ...
      ${_categories.length ? `<div style="padding:8px 14px 0;display:flex;gap:6px;overflow-x:auto;scrollbar-width:none;-webkit-overflow-scrolling:touch">
        <button onclick="ProductsPage.filterCat('')"
          style="flex-shrink:0;padding:5px 12px;border-radius:20px;border:1px solid ${!_filterCat?'var(--gold)':'var(--bdr)'};font-size:var(--fs-xs);font-weight:600;background:${!_filterCat?'var(--gold)':'var(--card)'};color:${!_filterCat?'#000':'var(--txt)'};cursor:pointer;white-space:nowrap">
          ทั้งหมด
        </button>
        ${_categories.map(c => `
          <button onclick="ProductsPage.filterCat('${_esc(c.name)}')"
            style="flex-shrink:0;padding:5px 12px;border-radius:20px;border:1px solid ${_filterCat===c.name?'var(--gold)':'var(--bdr)'};font-size:var(--fs-xs);font-weight:600;background:${_filterCat===c.name?'var(--gold)':'var(--card)'};color:${_filterCat===c.name?'#000':'var(--txt)'};cursor:pointer;white-space:nowrap">
            ${c.icon ? c.icon+' ' : ''}${_esc(c.name)}
          </button>`).join('')}
      </div>` : ''}
      <div id="prod-list" style="padding:10px 14px 0"></div>`;
    const inp = document.getElementById('prod-search');
    if (inp) { let t; inp.addEventListener('input', e => { clearTimeout(t); t = setTimeout(() => { _q = e.target.value; _renderAllList(); }, 200); }); }
    _renderAllList();
  }
  ...
  function _renderAllList() {
    ...
    let list = _filterCat ? _products.filter(p => (p.category||'') === _filterCat) : _products;
  ...
  function _prodRow(p) {
    ...
    const sc = stock <= 0 ? 'tag-red' : stock <= (p.min_stock_alert||5) ? 'tag-yellow' : 'tag-green';
    ...
    return `<div class="list-item" style="gap:10px" onclick="ProductsPage.edit('${p.id}')">
      ...
      <div class="li-right" style="align-items:flex-end">
        <span class="tag ${sc}">${_fmt(stock)} ชิ้น</span>
  ...
  function _renderReceive() {
    ...
    el.innerHTML = `
      <div style="padding:14px">
        <div class="section-title" style="margin-bottom:12px">บันทึกรับสินค้า</div>
        <div class="pm-field" style="margin-bottom:10px">
          ...
          <datalist id="rc-partner-list"></datalist>
        </div>
        ...
        <div class="section-title" style="margin-bottom:8px">
          รายการสินค้า
          <span class="section-link" onclick="ProductsPage.rcAddItem()">+ เพิ่มรายการ</span>
        </div>
        <datalist id="rc-prod-list">
          ${_products.map(p => `<option value="${_esc(p.name)}" data-id="${p.id}" data-sku="${_esc(p.sku||'')}" data-cost="${p.cost_price||0}">`).join('')}
        </datalist>
        <div id="rc-items"></div>
  ...
  async function _loadPartners() {
    if (_partners.length) {
      const dl = document.getElementById('rc-partner-list');
      if (dl) dl.innerHTML = _partners.map(p => `<option value="${_esc(p.company_name)}">`).join('');
      return;
    }
    try {
    ...
    el.innerHTML = _rcItems.map((it, i) => `
      <div style="display:grid;grid-template-columns:1fr 64px 76px 34px;gap:6px;align-items:center;margin-bottom:8px">
        ...
        <input type="number" value="${it.qty}" min="1" placeholder="จำนวน"
          onchange="ProductsPage.rcSetQty(${i},this.value)"
        ...
        <input type="number" value="${it.cost_price||''}" min="0" step="0.01" placeholder="ราคารับ"
          onchange="ProductsPage.rcSetCost(${i},this.value)"
  ...
  function _prodFormHtml(p) {
    ...
    return `<div style="padding:0 0 8px">
      ...
      ${p.image_url?`<div style="text-align:center;margin-bottom:10px"><img src="${_esc(p.image_url)}" style="height:72px;border-radius:10px;object-fit:cover"></div>`:''}
      <div class="pm-field" style="margin-bottom:10px">
        <label>URL รูปภาพ</label>
        <input type="url" id="pf-img" value="${_esc(p.image_url||'')}" placeholder="https://...">
      </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px">
        ...
        <div class="pm-field"><label>PV (Point Value)</label>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px">
        <div class="pm-field"><label>VAT</label>
          <select id="pf-vat" style="width:100%;background:var(--bg);border:1px solid var(--bdr);border-radius:8px;padding:9px 10px;color:var(--txt);font-size:var(--fs-sm);outline:none">
            <option value="no_vat"       ${(p.vat_type||'no_vat')==='no_vat'?'selected':''}>ไม่มี VAT</option>
            <option value="vat7"         ${p.vat_type==='vat7'?'selected':''}>VAT 7%</option>
            <option value="vat_included" ${p.vat_type==='vat_included'?'selected':''}>รวม VAT แล้ว</option>
          </select>
        </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px">
        ...
        <div class="pm-field"><label>แจ้งเตือนเมื่อต่ำกว่า</label>
          <input type="number" id="pf-alert" value="${p.min_stock_alert||5}" min="0" step="1">
        </div>
      </div>
  ...
  window.ProductsPage = {
    ...
    switchTab(tab) { _tab = tab; _q = ''; _filterCat = ''; _render(); },
    filterCat(cat) { _filterCat = cat; _renderAll(); },
    ...
    async save(id) {
      ...
      const payload = {
        ...
        vat_type:        document.getElementById('pf-vat')?.value               || 'no_vat',
        stock_qty:       parseFloat(document.getElementById('pf-stock')?.value)  || 0,
        min_stock_alert: parseInt(document.getElementById('pf-alert')?.value)    || 5,
    ...
    rcSetQty(i,v)  { _rcItems[i].qty=parseInt(v)||1; _rcCalcTotal(); },
    ...
    async saveReceive() {
      ...
      if (btn) { btn.disabled=true; btn.textContent='กำลังบันทึก...'; }
      try {
        await App.api('/api/pos/receive/create', {method:'POST',body:JSON.stringify(payload)});
