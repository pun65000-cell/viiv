(function() {
  ...
  async function _reload() {
    ...
    html += '<input id="bq" type="text" autocomplete="off" placeholder="ค้นหาเลขบิล / ลูกค้า" style="flex:1;min-width:140px;background:var(--card);border:1px solid var(--bdr);border-radius:10px;padding:9px 12px;color:var(--txt);font-size:var(--fs-sm);outline:none">';
  ...
  function _openDetail(b) {
    ...
    (b.items||[]).forEach(function(i) {
      ...
      items += '<div style="font-size:var(--fs-xs);color:var(--muted)">'+_esc(i.sku||'')+' · '+i.qty+' × ฿'+_fmt(i.price)+'</div></div>';
    ...
    html += '<div style="background:var(--bg);border-radius:10px;padding:10px 12px;margin-bottom:14px">'+summary+'</div>';
    html += '<div style="display:flex;gap:8px">';
    html += '<button onclick="BillPage.doPrint(\''+b.id+'\')" style="flex:1;background:var(--gold);color:#000;border:none;border-radius:10pstyle="margin-top:10px;font-size:var(--fs-xs);color:var(--muted)">หมายเหตุ: '+_esc(b.note)+'</div>';
    html += '</div>';
